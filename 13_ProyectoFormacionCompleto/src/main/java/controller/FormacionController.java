...
public class FormacionController {
	...
	public String altaAlumno(@ModelAttribute Alumno alumno, HttpServletRequest request) {
		return  (formacionService.altaAlumno(alumno))? "menu" : "AltaAlumno";
	}
	
	@PostMapping(value = "AltaCurso")
	public String altaCurso(@ModelAttribute Curso curso, HttpServletRequest request) {
	...
	public @ResponseBody List<Curso> cursosEntreFechas(@DateTimeFormat(pattern ="yyyy-MM-dd") @RequestParam("fecha1") Date fechaIni,
														@DateTimeFormat(pattern ="yyyy-MM-dd") @RequestParam("fecha2") Date fechaFin){
		
		return formacionService.consultarMatriculas(fechaIni, fechaFin);
	}
	
	@GetMapping(value="CursosNoAlumnos", produces = MediaType.APPLICATION_JSON_VALUE )
	public @ResponseBody List<Curso> cursosNoAlumnos(@RequestParam("usuario") String usuario){
